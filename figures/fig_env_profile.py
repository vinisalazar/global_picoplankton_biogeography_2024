...
def main():
    ...
    md = pd.read_csv("/local/path/to/data/metadata_1454_cluster_labels.csv", index_col=0)
    ...
    for i in range(data.shape[1]):
        transformed_column, lambda_value = yeojohnson(data[:, i] + 1)  # Adding 1 to handle zero values
    ...
    df_comp_standardized = pd.DataFrame(scaler.fit_transform(df_comp), columns=df_comp.columns)
    ...
    df_comp_decorrelated = pd.DataFrame(np.dot(df_comp_standardized, inv_cov_matrix), columns=df_comp.columns)
    ...
    df_all = pd.read_csv("/local/path/to/data/models/model_data/sourmash_k_10_1487_25m_not_scaled_16270117_points_X_test.csv", index_col=0)

    palette_composite_fig = {v["label"]: v["color"] for k, v in palettes["k_10"].items()}

    # Create a figure instance
    fig = plt.figure(figsize=(20, 18))

    # Define the grid specification
    gs = mpl.gridspec.GridSpec(4, 4, figure=fig, width_ratios=[1.2, 0.3, 0.3, 0.3], wspace=0.1, hspace=0.3)#, hspace=0.5, wspace=0.5)
    ...
    map_ax.add_feature(cfeature.NaturalEarthFeature("physical", "land", "10m"),
            ec="black", lw=0.5, alpha=1, color="#EFEFDB", zorder=1)
    ...
    for ix, row in md.iterrows():
        x, y, c, p = row["longitude"], row["latitude"], row["color"], row["sourmash_k_10_1487_25m"]
        ...
        line, = map_ax.plot(x, y, marker=m, markersize=10, color=c, zorder=3, markeredgecolor="k", alpha=0.85)
        handle = mpl.lines.Line2D([], [], color="black", marker=m, markersize=10, label=p)
    legend_handles = [(palettes["k_10"][i]["marker"], palettes["k_10"][i]["label"], palettes["k_10"][i]["color"]) for i in [14, 16, 10, 2, 3, 7, 5, 11, 9, 0]]
    legend_handles = [mpl.lines.Line2D([], [], color=c, linestyle="None", marker=m, markersize=12, markeredgecolor="black", markeredgewidth=1, label=p) for (m, p, c) in legend_handles]
    ...
    _, _ = model.biplot(c=md["color"],
                        marker=md["sourmash_k_10_1487_25m"].map({k: v["marker"] for k, v in palettes["k_10"].items()}).values,
                        ax=biplot_ax,
                        fontweight="normal",
                        legend=None,
                        title=None,
    ...
    axes = [(0, 1),
            ...
            (3, 2),
            (3, 3)]
    env_cols = ["OceanTemperature", "Salinity", "Nitrate",
                "Phosphate", "DissolvedMolecularOxygen", "SeaIceCover",
                "pH", "Silicate", "Chlorophyll", "DissolvedIron"]
    units = {
    ...
    del(df_all)
    plot_data["province"] = plot_data["province"].astype(int).map({k: v["label"] for k, v in palettes["k_10"].items()})
    for col, (i, j) in zip(env_cols, axes):
        ...
        sns.boxenplot(plot_data, x=col, y="province", ax=ax, hue="province", palette=palette_composite_fig, order = [palettes["k_10"][i]["label"] for i in [14, 16, 10, 2, 3, 7, 5, 11, 9, 0]], showfliers=False, zorder=2)
        ax.set_ylabel("")
        ...
        ax.xaxis.set_major_locator(mpl.ticker.MaxNLocator(nbins=5, min_n_ticks=4, integer=True))
        ax.set_xticklabels(ax.get_xticklabels(), fontsize=12)
        unit = f" ({units[col]})" if len(units[col]) > 0 else ""
        ax.set_xlabel(f"{split_on_capitals(col)}{unit}", fontsize=14)
        ax.xaxis.label.set_size(12)
        ax.xaxis.set_label_position("top")
    fig.text(0.105, 0.89, "\\textbf{A}", fontsize=20, fontweight="bold", ha="left", va="top")
    fig.text(0.105, 0.47, "\\textbf{B}", fontsize=20, fontweight="bold", ha="left", va="top")
    fig.text(0.54, 0.89, "\\textbf{C}", fontsize=20, fontweight="bold", ha="left", va="top")

    outfile = "/local/path/to/figures/img/final_vectors/fig2_env.pdf"
    plt.savefig(outfile, dpi=600 if outfile[-3:] == "png" else "figure", bbox_inches="tight")
