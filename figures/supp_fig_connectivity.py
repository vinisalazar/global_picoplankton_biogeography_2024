...
distances = distances.reset_index(names=["sample1"]).melt(
    id_vars="sample1", var_name="sample2", value_name="distance"
)
distances = distances[distances["distance"] == distances["distance"]].reset_index(
    drop=True
)
...
for ix, row in md.iterrows():
    ax.plot(
        row["longitude"],
        row["latitude"],
        color=palettes["k_10"][row["sourmash_k_10_1487_25m"]]["color"],
        marker=palettes["k_10"][row["sourmash_k_10_1487_25m"]]["marker"],
        markersize=10,
        markeredgewidth=0.5,
...
for ix, row in plot_data.iterrows():
    ...
    if row["alpha"] <= quantiles.loc[quantile_values[0]]:
        ...
        color = "blue"
    ax.plot(
        [md.loc[row["sample1"], "longitude"], md.loc[row["sample2"], "longitude"]],
        [md.loc[row["sample1"], "latitude"], md.loc[row["sample2"], "latitude"]],
        color=color,
        alpha=alpha,
        lw=0.5,
        zorder=3,
...
plt.savefig(
    "/local/path/to/figures/final_draft_imgs/figS7_connectivity.svg",
    bbox_inches="tight",
)
