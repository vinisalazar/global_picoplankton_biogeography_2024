...
counts[rank] = pd.read_csv(
    f"/local/path/to/data/counts/{rank}_trimmed_mean_formatted_clean_normalised.csv",
    index_col=0,
)
plotIndiv_c1_2[rank] = pd.read_csv(
    f"{data_dir}/splsda_all_{rank}_ncompc1_2_keepX{keepX}_indiv.csv", index_col=0
)
plotIndiv_c3_4[rank] = pd.read_csv(
    f"{data_dir}/splsda_all_{rank}_ncompc3_4_keepX{keepX}_indiv.csv", index_col=0
)
plotIndiv_c1_2[rank]["group"] = (
    plotIndiv_c1_2[rank]["group"]
    .str.replace("SSTC", "OTEM")
    .str.replace("TCON", "TGYR")
    .str.replace("UPWL", "CTEM")
    .str.replace("MEDI", "MTEM")
    .str.replace("NADR", "STEM")
)
plotIndiv_c3_4[rank]["group"] = (
    plotIndiv_c3_4[rank]["group"]
    .str.replace("SSTC", "OTEM")
    .str.replace("TCON", "TGYR")
    .str.replace("UPWL", "CTEM")
...
for contrib in ("max",):
    for comp in (1, 2, 3, 4):
        df = pd.read_csv(
            f"{data_dir}/splsda_all_KEGG_Pathway_comp{comp}_keepX{keepX}_loadings_median_contrib_{contrib}.csv",
            index_col=0,
        )
        df["abs_importance"] = df["importance"].apply(abs)
        df = df.sort_values("abs_importance").reset_index()
        df.columns = [
            i.replace("SSTC", "SANT").replace("TCON", "TGYR").replace("UPWL", "CTEM")
            for i in df.columns
        ]
        df["GroupContrib"] = (
            df["GroupContrib"]
            .str.replace("SSTC", "SANT")
            .str.replace("TCON", "TGYR")
            .str.replace("UPWL", "CTEM")
        )
...
def main():
    ...
    plotIndiv_c1_2_ax.yaxis.set_label_coords(-0.15, 0.4)
    ...
    plotIndiv_c3_4_ax.yaxis.set_label_coords(-0.15, 0.5)
    ...
    legend = plotIndiv_c1_2_ax.legend(
    ...
    for comp in (1, 2, 3, 4):
        ...
        ax.set_title(
            f"X-variate {comp - variate_offset} loading importance. Comp. {comp}",
            fontsize=14,
        )
        ax.set_xlabel("")
        ax.set_ylabel("")
    ...
    outfile = (
        f"/local/path/to/figures/img/final_vectors/fig4_func_profile_keepX{keepX}.svg"
    )
