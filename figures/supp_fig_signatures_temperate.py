...
"""
Plot selected variables from sPLS-DA for temperate provinces.
"""
...
cim = pd.read_csv(
    f"{wd}/provinces_final/data/R/block_splsda_temp_genomes_Pathway_KO_Environment_keepX{keepX}_ncomp_2_cim.csv",
    index_col=0,
)
plotIndiv = pd.read_csv(
    f"{wd}/provinces_final/data/R/block_splsda_temp_genomes_Pathway_KO_Environment_keepX{keepX}_ncomp_2_plotIndiv.csv",
    index_col=0,
)
plotVar = pd.read_csv(
    f"{wd}/provinces_final/data/R/block_splsda_temp_genomes_Pathway_KO_Environment_keepX{keepX}_ncomp_2_plotVar.csv",
    index_col=0,
)
genomes = pd.read_csv(
    f"{wd}/provinces_final/data/counts/genomes_trimmed_mean_formatted_clean_normalised.csv",
    index_col=0,
)
kegg_ko = pd.read_csv(
...
plotIndiv["group"] = (
    plotIndiv["group"].str.replace("UPWL", "CTEM").str.replace("SSTC", "SANT")
)
...
sample_markers = {"SANT": "<", "NADR": ">", "MEDI": "p", "CTEM": "P"}
var_markers = {
    "Taxonomy": "D",
    "Environment": "X",
    "Function_Pathway": "o",
...
labels_to_add = {
    # # Top center
    28: {"x": 0.05, "y": 0.1, "arrow": True, "remove_str": " permease protein_5706"},
    26: {"x": 0.05, "y": 0.15, "arrow": True, "remove_str": " atp-binding protei_5054"},
    # 20: {"x": -0.2, "y": -0.2, "arrow": True},
    # 15: {"x": -0.5, "y": 0.12, "arrow": True, "remove_str": "and other terpenoid-quinone "},
    16: {"x": -0.4, "y": 0.085, "arrow": True},
    12: {"x": -0.5, "y": -0.125, "arrow": True},
    4: {"x": -0.2, "y": -0.175, "arrow": True, "ranks": "order family genus"},
    10: {"x": 0, "y": -0.125, "arrow": True, "ranks": "order family genus"},
    1: {"x": -0.6, "y": -0.1, "arrow": True, "ranks": "order family genus"},
    # # Center right
    5: {"x": 0.1, "y": 0.05, "arrow": True, "ranks": "family genus"},
    11: {"x": -0.1, "y": 0.15, "arrow": True},
    17: {"x": 0.0, "y": -0.15, "arrow": True},
    18: {"x": 0.15, "y": 0.1, "arrow": True},
    19: {"x": 0.0, "y": -0.15, "arrow": True},
    # # Center left
    2: {"x": -0.1, "y": -0.15, "arrow": True, "ranks": "genus"},
    3: {"x": 0.1, "y": 0.05, "arrow": True, "ranks": "order family genus"},
    # Environment
...
plt.savefig("../final_draft_imgs/supp_fig_temp_signatures.pdf", bbox_inches="tight")


plotVar_tax = plotVar.merge(
    gmd[["domain", "phylum", "class", "order", "family", "genus", "species"]],
    left_on="p.names",
    right_index=True,
    how="left",
)
