...
smd = pd.read_csv(
    f"{data_dir}/provinces_final/data/metadata_1454_cluster_labels.csv", index_col=0
)

smd["cluster"] = smd["sourmash_k_10_1487_25m"].map(
    {k: v["label"] for k, v in palettes["k_10"].items()}
)
...
ax.set_title(
    "Cross-validation strategy: repeated stratified \\textit{k}-fold. 9 splits, 10 repeats.\nRF params: Max._depth=10, Min._samples_split=5, Min._samples_leaf=2, Max._samples=0.7".replace(
        "_", " "
    ),
    loc="left",
)
...
def plot_roc_curve(
    ...
    set_ylabel=True,
    set_xlabel=True,
    title="Receiver Operating Characteristic (ROC) - Multiclass",
):
    clf.fit(X_train, y_train)
    clf.classes_ = classes

    y_score = (
        clf.predict_proba(X_test) if not binary_clf else clf.predict_proba(X_test)[:, 1]
    )
...
plot_roc_curve(
    ...
    set_xlabel=False,
    set_ylabel=False,
    title="Receiver Operating Characteristic (ROC) curve – all provinces",
)

# Temperate classes
prov_cat_df_temp = smd[smd["cluster"].isin("CTEM MTEM STEM OTEM".split())]
X_temp = prov_cat_df_temp.drop_duplicates("coords")[env_cols]
y_temp = prov_cat_df_temp.drop_duplicates("coords")["sourmash_k_10_1487_25m"].astype(
    str
)
...
plot_roc_curve(
    ...
    classes_trop,
    ax4,
    title="ROC curve – Tropical",
)

ax2.tick_params(labelbottom=False)
ax3.tick_params(labelbottom=False)
plt.annotate(
    "\\textbf{A}", (0.05, 0.81), xycoords="figure fraction", weight="bold", size=24
)
plt.annotate(
    "\\textbf{B}", (0.62, 0.81), xycoords="figure fraction", weight="bold", size=24
)

# plt.savefig("/local/path/to/figures/final_draft_imgs/figS3_confusion_matrix_roc_curve.svg", bbox_inches="tight")
plt.savefig(
    "/local/path/to/figures/final_draft_imgs/figS3_confusion_matrix_roc_curve.png",
    dpi=600,
    bbox_inches="tight",
)
...
class_feature_importances_dict = {
    class_: pd.DataFrame(
        pd.DataFrame(class_feature_importances).loc[:, class_].to_list()
    )
...
feature_importances.columns = [
    split_on_capitals(i) for i in feature_importances.columns
]
...
ax2.yaxis.set_ticklabels(
    ax2.yaxis.get_ticklabels(), fontsize=10
)  # Adjust fontsize as needed

plt.annotate(
    "\\textbf{A}", (0.05, 0.80), xycoords="figure fraction", fontweight="bold", size=16
)
plt.annotate(
    "\\textbf{B}", (0.515, 0.80), xycoords="figure fraction", fontweight="bold", size=16
)
plt.annotate(
    "\\textbf{C}", (0.05, 0.4), xycoords="figure fraction", fontweight="bold", size=16
)
...
for ax_, (class_, df) in zip(class_axes, class_feature_importances_dict.items()):
    ...
    if ax_ is ax10:
        ax_.set_xlabel(
            "Feature relative importance per class (One vs. Rest classifier)"
        )
    sns.boxenplot(
        data=df[feature_importances_order],
        orient="h",
        ax=ax_,
        color=colour_dict[class_],
    )
    ax_.set_title(class_)

plt.savefig(
    "/local/path/to/figures/final_draft_imgs/figS4_feature_importance_correlation.svg",
    bbox_inches="tight",
)
