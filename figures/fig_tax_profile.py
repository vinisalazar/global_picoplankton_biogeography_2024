...
smd = pd.read_csv(
    f"{wd}/provinces_final/data/metadata_1454_cluster_labels.csv",
    index_col=0,
    dtype={"sourmash_k_10_1487_25m": "object"},
)
gmd = pd.read_csv(
...
tables_clean = {
    "genomes": pd.read_csv(
        f"{wd}/provinces_final/data/counts/genomes_trimmed_mean_formatted_clean.csv",
        index_col=0,
    ),
    "genus": pd.read_csv(
        f"{wd}/provinces_final/data/counts/genus_trimmed_mean_formatted_clean.csv",
        index_col=0,
    ),
    "family": pd.read_csv(
        f"{wd}/provinces_final/data/counts/family_trimmed_mean_formatted_clean.csv",
        index_col=0,
    ),
    "class": pd.read_csv(
        f"{wd}/provinces_final/data/counts/class_trimmed_mean_formatted_clean.csv",
        index_col=0,
    ),
    "order": pd.read_csv(
        f"{wd}/provinces_final/data/counts/order_trimmed_mean_formatted_clean.csv",
        index_col=0,
    ),
    "phylum": pd.read_csv(
        f"{wd}/provinces_final/data/counts/phylum_trimmed_mean_formatted_clean.csv",
        index_col=0,
    ),
...
cim = pd.read_csv(
    f"{wd}/provinces_final/data/R/drafts/splsda_all_genomes_ncomp10_keepX3_cim.csv",
    index_col=0,
)
...
tables_clean["genus"]["g__GCA_902624425_gen_nov"] = tables_clean["genomes"][
    "GCA_902624425"
]
...
for rank_ in ranks[:-1]:
    for genome in tables_clean["genomes"].columns:
        ...
        if parent == f"{rank_[0]}__":
            ...
            tables_clean[rank_][f"{rank_[0]}__{genome}"] = tables_clean["genomes"][
                genome
            ]

tables_clean["domain"] = (
    tables_clean["genomes"].T.join(gmd["domain"]).groupby("domain").sum().T
)
...
tab20_colors_bar = (
    mpl.colormaps["tab20"].colors[::2] + mpl.colormaps["tab20"].colors[1::2]
)
...
def get_parent(taxa, rank):
    ...
    return (
        gmd.reset_index(names=["genomes"])[
            gmd.reset_index(names=["genomes"])[rank] == taxa
        ]
        .drop_duplicates(rank)[parent_rank]
        .values[0]
    )
...
def create_brightness_colormap(
    hex_color, brightness_factor=0.8, name="custom_brightness"
):
    """
    ...
    if isinstance(hex_color, str):
        start_color = (
            ...
            else hex_to_rgb(mpl.colors.to_hex(hex_color))
        )
...
def create_pie_data(
    ...
    colormaps=colormaps,
    add_archaea=False,
    add_other_phyla=False,
    add_other_phyla_N=10,
):
    ...
    if add_archaea:
        most_abun += (
            pie_data.median()
            .loc[gmd[gmd["domain"] == "d__Archaea"].index]
            .nlargest(2)
            .index.tolist()
        )
    if add_other_phyla:
        most_abun += (
            pie_data.median()
            .loc[gmd[~gmd["phylum"].isin(add_other_phyla)].index]
            .nlargest(add_other_phyla_N)
            .index.tolist()
        )
    ...
    pie_data["parent"] = pie_data.apply(
        lambda x: get_parent(x["taxa"], x["rank"]), axis=1
    )
    ...
    pie_data["color"] = pie_data.apply(
        lambda row: get_phylum(row["taxa"], row["rank"]), axis=1
    ).map(colormaps)
    pie_data["color"] = pie_data.apply(
        lambda row: get_colormap_colors(row["color"], len(ranks))[
...
stacked_bar_phylum["sourmash_k_10_1487_25m"] = (
    stacked_bar_phylum["sourmash_k_10_1487_25m"]
    .astype(int)
    .map({k: v["label"] for k, v in palettes["k_10"].items()})
)
stacked_bar_phylum["sourmash_k_10_1487_25m"] = pd.Categorical(
    stacked_bar_phylum["sourmash_k_10_1487_25m"],
...
for category in stacked_bar_phylum["sourmash_k_10_1487_25m"].unique():
    sort_order_phylum[category] = get_ordered_labels(
        stacked_bar_phylum[
            stacked_bar_phylum["sourmash_k_10_1487_25m"] == category
        ].drop("sourmash_k_10_1487_25m", axis=1)
    )
stacked_bar_phylum = stacked_bar_phylum.groupby(
    "sourmash_k_10_1487_25m", group_keys=False
).apply(
    lambda x: sort_by_order(x, sort_order_phylum[x["sourmash_k_10_1487_25m"].iloc[0]])
)
stacked_bar_phylum.columns = [
    i[3:] if i[1] == "_" else i for i in stacked_bar_phylum.columns
]


colordict_phylum = {
    k: colormaps_bar["p__" + k]
    for k in stacked_bar_phylum.columns[:-1]
    if k != "Others"
}
...
stacked_bar_family["sourmash_k_10_1487_25m"] = (
    stacked_bar_family["sourmash_k_10_1487_25m"]
    .astype(int)
    .map({k: v["label"] for k, v in palettes["k_10"].items()})
)
stacked_bar_family["sourmash_k_10_1487_25m"] = pd.Categorical(
    stacked_bar_family["sourmash_k_10_1487_25m"],
    categories="APLR BPLR BALT CTEM OTEM STEM MTEM TGYR TRHI TRLO".split(),
    ordered=True,
)
...
for category in stacked_bar_family["sourmash_k_10_1487_25m"].unique():
    sort_order_family[category] = get_ordered_labels(
        stacked_bar_family[
            stacked_bar_family["sourmash_k_10_1487_25m"] == category
        ].drop("sourmash_k_10_1487_25m", axis=1)
    )
stacked_bar_family = stacked_bar_family.groupby(
    "sourmash_k_10_1487_25m", group_keys=False
).apply(
    lambda x: sort_by_order(x, sort_order_family[x["sourmash_k_10_1487_25m"].iloc[0]])
)
stacked_bar_family.columns = [
    i[3:] if i[1] == "_" else i for i in stacked_bar_family.columns
]
...
colordict_family = {
    k: colormaps_bar[
        gmd.drop_duplicates("family").set_index("family").loc[f"f__{k}", "phylum"]
    ]
    for k in stacked_bar_family.columns[:-1]
    if k != "Others"
}
...
for phylum, phylum_color in colordict_phylum.items():
    ...
    for family, family_color in colordict_family.items():
        if "f__" + family in gmd[gmd["phylum"] == f"p__{phylum}"]["family"].to_list():
            colordict_family[family] = adjust_brightness(
                family_color, phylum_brightness
            )
            phylum_brightness += 0.15


rename_families = (
    gmd.drop_duplicates("family")
    .set_index("family")
    .loc[["f__" + i for i in stacked_bar_family.columns[:-2]]]
    .iloc[[3, 6, 7, 8]]["order"]
    .to_dict()
)
...
renamed_colordict_family = {
    rename_families[k]: v
    for k, v in colordict_family.items()
    if k in rename_families.keys()
}
colordict_family = {**colordict_family, **renamed_colordict_family}
colordict_family = {
    k: colordict_family[k] for k in stacked_bar_family.columns[:-1] if k != "Others"
}
colordict_family = {
    k: v for k, v in colordict_family.items() if k in stacked_bar_family.columns
}
colordict_family["Others"] = "#9a9a9a"


median_archaea = (
    get_rel_abun(tables_clean["phylum"])[
        gmd[gmd["domain"] == "d__Archaea"]["phylum"].unique()
    ]
    .sum(axis=1)
    .to_frame()
    .merge(smd["sourmash_k_10_1487_25m"], left_index=True, right_index=True)
    .groupby("sourmash_k_10_1487_25m")
    .median()
    .reset_index()
)
median_archaea.columns = ["province", "archaea"]
median_archaea["province_label"] = (
    median_archaea["province"]
    .astype(int)
    .map({k: v["label"] for k, v in palettes["k_10"].items()})
)
...
for p in "14 16 10 2 3 7 5 11 9 0".split():
    ...
    add_archaea = (
        True
        if p in median_archaea[median_archaea["archaea"] >= 0.02]["province"].unique()
        else False
    )
    add_other_phyla = (
        False
        if p not in "7 14 0 10 2".split()
        else ["p__Proteobacteria", "p__Bacteroidota"]
    )
    add_other_phyla = (
        add_other_phyla
        if p != "3"
        else ["p__Proteobacteria", "p__Bacteroidota", "p__Cyanobacteria"]
    )
    provinces_pie_data[p] = create_pie_data(
        province=p,
        add_archaea=add_archaea,
        add_other_phyla=add_other_phyla,
        add_other_phyla_N=10,
    )
...
for i, (column, color) in enumerate(colordict_phylum.items()):
    ax_phylum.bar(
        indices_phylum,
        stacked_bar_phylum[column],
        bar_width,
        bottom=bottom_phylum,
        label=column,
        color=color,
    )
...
for category in unique_categories:
    category_indices = stacked_bar_phylum.index[
        stacked_bar_phylum["sourmash_k_10_1487_25m"] == category
    ].tolist()
    start_pos = indices_phylum[stacked_bar_phylum.index.get_loc(category_indices[0])]
    end_pos = (
        indices_phylum[stacked_bar_phylum.index.get_loc(category_indices[-1])]
        + bar_width
    )
...
ax_phylum.set_xlim(
    min(indices_phylum) - bar_width / 2, max(indices_phylum) + bar_width / 2
)
...
for pos, category in zip(category_positions, unique_categories):
    ax_phylum.text(
        pos,
        -0.05 * bottom_phylum.max(),
        category,
        ha="center",
        va="top",
        backgroundcolor=label_face_colors[category],
        fontsize=8,
    )
...
rectangles = []
for category in unique_categories:
    category_indices = stacked_bar_family.index[
        stacked_bar_family["sourmash_k_10_1487_25m"] == category
    ].tolist()
    start_pos = indices_family[stacked_bar_family.index.get_loc(category_indices[0])]
    end_pos = (
        indices_family[stacked_bar_family.index.get_loc(category_indices[-1])]
        + bar_width
    )
    midpoint = (start_pos + end_pos) / 2
    category_positions.append(midpoint)
...
ax_family.set_xlim(
    min(indices_family) - bar_width / 2, max(indices_family) + bar_width / 2
)
...
for ax, province in zip(pie_charts, "14 10 2 3 7 0 16 5 11 9".split()):
    ...
    sunburst(
        provinces_pie_data[province],
        ax,
        remove_labels=True,
        remove_pct=True,
        pctdistance=0.8,
        format_lineage=format_lineage,
        rotatelabels=False,
        genome_fontsize=6,
        remove_all_labels=True,
    )
    ax.set_title(
        palettes["k_10"][int(province)]["label"], bbox=dict(facecolor=color, alpha=0.7)
    )
...
plt.savefig(
    "/local/path/to/figures/final_draft_imgs/fig3_tax_top_genomes_only.png",
    dpi=1200,
    bbox_inches="tight",
    transparent=True,
)
...
genome_labels = gmd.loc[cim.columns, [i for i in ranks if i != "genomes"]].apply(
    lambda row: "; ".join(
        list(dict.fromkeys([i[3:] if i[1] == "_" else i for i in row] + [row.name]))
    ),
    axis=1,
)
...
col_colors = (
    smd.loc[cim.index]["sourmash_k_10_1487_25m"]
    .astype(int)
    .map({k: v["color"] for k, v in palettes["k_10"].items()})
    .rename("Province")
)


cimplot = sns.clustermap(
    cim.T,
    method="average",
    cmap="Spectral_r",
    xticklabels=False,
...
gmd.drop_duplicates("genus").set_index("genus").loc[
    tables_clean["genus"].median().nlargest(20).index,
    [i for i in ranks if i != "genus"],
]


gmd.drop_duplicates("genus").set_index("genus").loc[
    tables_clean["genus"].median().nlargest(100).index,
    [i for i in ranks if i != "genus"],
]
...
s = pd.Series(
    diversity.alpha_diversity(
        metric, get_rel_abun(tables_clean[rank]).map(lambda x: 0.0 if x < 1e-6 else x)
    )
)
...
for province in "0 14 16 10 2 3 7 5 11 9".split():
    ...
    ax.set_title(
        palettes["k_10"][int(province)]["label"],
        bbox=dict(facecolor=palettes["k_10"][int(province)]["color"], alpha=0.7),
    )
    print()


gmd.loc[
    provinces_pie_data["0"].query("rank == 'genomes'")["taxa"],
    ranks[:-1] + ["sci_names"],
]
...
fig, ax = plt.subplots(1, 1, figsize=(10, 10))
sunburst(
    provinces_pie_data["0"],
    ax,
    remove_labels=True,
    remove_pct=False,
    pctdistance=0.8,
    format_lineage=lambda x: custom_join(
        gmd.loc[x, ranks[:-1]].str[3:].to_list() + [x]
    ),
)
ax.set_title(
    palettes["k_10"][int("0")]["label"],
    bbox=dict(facecolor=palettes["k_10"][int("0")]["color"], alpha=0.7),
)
...
sunburst(
    provinces_pie_data["0"], ax, remove_labels=True, remove_pct=False, pctdistance=0.8
)
...
sunburst(
    provinces_pie_data["7"], ax, remove_labels=True, remove_pct=False, pctdistance=0.8
)
...
sunburst(
    provinces_pie_data["10"], ax, remove_labels=True, remove_pct=False, pctdistance=0.8
)
...
sunburst(
    provinces_pie_data["5"], ax, remove_labels=True, remove_pct=False, pctdistance=0.8
)
...
sunburst(
    provinces_pie_data["7"],
    ax,
    remove_labels=True,
    remove_pct=False,
    pctdistance=0.8,
    format_lineage=lambda text_: custom_join(
        gmd.loc[text_, ranks[:-1]].map(lambda x: x[3:]).to_list() + [text_]
    ),
)
...
data_ = (
    get_rel_abun(
        tables_clean["genomes"][
            gmd[
                (gmd["genus"].str.contains("Synechococcus"))
                | (gmd["genus"].str.contains("Cyanobium"))
            ].index
        ]
    )
    .join(
...
data_ = data_.rename(
    columns={
        **gmd.loc[data_.columns[:-1], "sci_names"].to_dict(),
        **{"GCA_000012505": "Synechococcus sp. CC9902"},
    }
)
sns.clustermap(
    data_.T,
    col_colors=col_colors.loc[data_.index],
    cmap="Spectral_r",
    figsize=(7.5, 5),
    xticklabels=False,
    yticklabels=True,
...
plt.savefig(
    "/local/path/to/figures/final_draft_imgs/clustermap_synechococcus.svg",
    bbox_inches="tight",
)
...
data_ = (
    get_rel_abun(
        tables_clean["genus"][
            gmd[(gmd["genus"].str.contains("Prochlorococcus"))]["genus"].unique()
        ]
    )
    .join(
        smd["sourmash_k_10_1487_25m"]
...
plt.savefig(
    "/local/path/to/figures/final_draft_imgs/clustermap_prochlorococcus_genus.svg",
    bbox_inches="tight",
)


data_ = get_rel_abun(
    tables_clean["genomes"][gmd[(gmd["genus"].str.contains("Prochlorococcus_A"))].index]
)
...
data_ = (
    data_.rename(columns=rename_dict)
    .join(
        smd["sourmash_k_10_1487_25m"]
        .astype(int)
        .map({k: v["label"] for k, v in palettes["k_10"].items()})
...
plt.savefig(
    "/local/path/to/figures/final_draft_imgs/clustermap_prochlorococcus_A_genome.svg",
    bbox_inches="tight",
)


data_ = get_rel_abun(
    tables_clean["genomes"][gmd[(gmd["genus"].str.contains("Prochlorococcus"))].index]
)
...
plt.savefig(
    "/local/path/to/figures/final_draft_imgs/clustermap_prochlorococcus_genome.svg",
    bbox_inches="tight",
)
...
data_ = get_rel_abun(tables_clean["genomes"])[
    gmd[(gmd["family"].str.contains("Pelagibacteraceae"))].index
]
data_ = data_[data_.max().nlargest(10).index]
rename_dict = (
    gmd.loc[data_.columns, ["family", "genus", "sci_names"]]
...
plt.savefig(
    "/local/path/to/figures/final_draft_imgs/clustermap_pelagibacter.eps",
    bbox_inches="tight",
)
...
data_ = get_rel_abun(
    tables_clean["genomes"][gmd[(gmd["family"].str.contains("SAR86"))].index]
)
...
plt.savefig(
    "/local/path/to/figures/final_draft_imgs/clustermap_sar86.svg", bbox_inches="tight"
)


data_ = get_rel_abun(
    tables_clean["genomes"][gmd[(gmd["family"].str.contains("HIMB59"))].index]
)
# data_ = data_[data_.median().nlargest(10).index]
rename_dict = (
    gmd.loc[data_.columns, ["family", "genus", "sci_names"]]
...
plt.savefig(
    "/local/path/to/figures/final_draft_imgs/clustermap_himb59.svg", bbox_inches="tight"
)
...
plt.savefig(
    "/local/path/to/figures/final_draft_imgs/clustermap_synechococcus.png",
    dpi=600,
    bbox_inches="tight",
)
...
plt.savefig(
    "/local/path/to/figures/final_draft_imgs/clustermap_prochlorococcus_genus.png",
    dpi=600,
    bbox_inches="tight",
)
...
plt.savefig(
    "/local/path/to/figures/final_draft_imgs/clustermap_prochlorococcus_A_genome.png",
    dpi=600,
    bbox_inches="tight",
)
...
plt.savefig(
    "/local/path/to/figures/final_draft_imgs/clustermap_prochlorococcus_genome.png",
    dpi=600,
    bbox_inches="tight",
)
...
plt.savefig(
    "/local/path/to/figures/final_draft_imgs/clustermap_pelagibacter.png",
    dpi=600,
    bbox_inches="tight",
)
...
plt.savefig(
    "/local/path/to/figures/final_draft_imgs/clustermap_sar86.png",
    dpi=600,
    bbox_inches="tight",
)
...
plt.savefig(
    "/local/path/to/figures/final_draft_imgs/clustermap_himb59.png",
    dpi=600,
    bbox_inches="tight",
)
