...
def old_plot_colored_markers(
    sample_metadata,
    color_category="depth",
    cmap="tab20c",
    jitter=0.05,
    legend=True,
    title=None,
    ...
    color_dict = {
        category: cmap(i)
        for i, category in enumerate(sample_metadata[color_category].unique())
    }
    ...
    if legend:
        patches = {
            str(label): matplotlib.patches.Patch(color=color, label=label)
            for label, color in color_dict.items()
        }
        try:
            patches = {
                f"{k} (N = {sample_metadata[color_category].value_counts().loc[int(k)]})": v
                for k, v in sorted(patches.items())
            }
        except ValueError:
            patches = {
                f"{k} (N = {sample_metadata[color_category].value_counts().loc[k]})": v
                for k, v in sorted(patches.items())
            }
...
def plot_colored_markers(
    ...
    if palette is not None:
        ...
        color_dict = {
            ...
            if k in sample_metadata[color_category].unique()
        }
...
get_cmap_ix_tab20c = lambda ix: matplotlib.colors.rgb2hex(
    plt.get_cmap("tab20c").colors[ix]
)
