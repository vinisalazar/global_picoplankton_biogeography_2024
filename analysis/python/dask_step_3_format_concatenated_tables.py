...
mappings = {
    ...
    "KEGG_ko": pd.read_csv(
        "/local/path/to/data/misc/renaming/ko_entries.tsv",
        sep="\t",
        index_col=0,
        header=None,
    )
    ...
    "KEGG_Pathway": pd.read_csv(
        "/local/path/to/data/misc/renaming/pathway_entries.tsv",
        sep="\t",
        index_col=0,
        header=None,
    )
    ...
    "KEGG_Reaction": pd.read_csv(
        "/local/path/to/data/misc/renaming/reaction_entries.tsv",
        sep="\t",
        index_col=0,
        header=None,
        on_bad_lines="skip",
    ...
    "KEGG_rclass": pd.read_csv(
        "/local/path/to/data/misc/renaming/rclass_entries.tsv",
        sep="\t",
        index_col=0,
        header=None,
    )
...
mappings["KEGG_rclass"] = {
    (k[3:] if k.startswith("rc:") else k): v for k, v in mappings["KEGG_rclass"].items()
}
mappings["KEGG_rclass"] = {
    k: (k if v == "[]" else v) for k, v in mappings["KEGG_rclass"].items()
}
kegg_ko_2nd_attempt = (
    pd.read_csv(
        "/local/path/to/data/misc/renaming/ko_entries_2nd_try.csv",
        sep="\t",
        index_col=0,
        header=None,
    )
...
def process_data(input_dir, output_dir, force=False, mode="tax"):
    ...
    if mode == "tax":
        ...
        ranks = (
            "BRITE COG_category KEGG_Reaction KEGG_rclass KEGG_ko KEGG_Pathway".split()
        )
        ...
        for metric in tqdm(metrics):
            ...
            df["sample_name"] = (
                ...
                .apply(
                    lambda l: (
                        l[0]
                        if (not l[0].startswith("Arc")) and (len(l) > 1)
                        else "_".join(l)
                    )
                )
            )
            df["sample_name"] = df["sample_name"].apply(
                lambda s: "_".join(s.split("_")[:-1]) if len(s.split("_")) > 3 else s
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--input_dir",
        type=str,
        help="Path to the input directory containing the CSV files.",
    )
    parser.add_argument(
        "-o",
        "--output_dir",
        type=str,
        help="Path to the output directory for saving formatted files.",
    )
    parser.add_argument(
        "-m",
        "--mode",
