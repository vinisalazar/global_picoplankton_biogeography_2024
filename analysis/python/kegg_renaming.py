...
files = {
    Path(f).stem.replace("_trimmed_mean_formatted_clean_normalised", ""): f
    for f in files
    if "KEGG" in f
}
files = {
    k: pd.read_csv(v, index_col=0, nrows=1).columns.to_list() for k, v in files.items()
}
for k, v in files.items():
    print(k, len(v))

with open(
    "/data/gpfs/projects/punim1293/vini/db/kegg/genes/ko/ko",
    "r",
    encoding="utf-8",
    errors="replace",
) as ko_file:
...
ko_entries = {
    k: v.replace("NAME        ", "").split(" [EC")[0] for k, v in ko_entries.items()
}
ko_entries = {
    i: ko_entries[i[3:]] for i in files["KEGG_ko"] if i[3:] in ko_entries.keys()
}
ko_missing = [i for i in files["KEGG_ko"] if i not in ko_entries.keys()]
ko_all = {**ko_entries, **{i: i for i in ko_missing}}

missing = pd.read_csv(
    "/local/path/to/data/misc/renaming/ko_missing_2nd_try.csv", header=None
)[0].to_list()
